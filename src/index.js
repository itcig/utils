/**
 * Functions used internally for repo build and maintenance
 *
 * @type {module:fs}
 */
const fs = require('fs');
...
function createAliases(cwd, sourceDirectory) {
  ...
  const alias = result.workspaces
    ...
    .filter(Boolean)
    .reduce((acc, { pkgDirectory, packageJson, workspaceName, packageFolder }) => {
...
function testCoverage(rootDir, testCovPath) {
  const { packageJsonPath, packageJson: pkg, isMonorepo: isMono, fromRoot, cwd } = getWorkspacesAndExtensions(rootDir);
  ...
  const lcovReportPath =
    typeof testCovPath === 'string' ? fromRoot(testCovPath) : fromRoot('coverage', 'lcov-report', 'index.html');

  const LCOV_REPORT = fromRoot(lcovReportPath);

  if (!fs.existsSync(LCOV_REPORT)) {
    const file = path.relative(cwd, LCOV_REPORT);
    throw new Error(`Run tests with coverage. Missing coverage report ./${file}!`);
  }
  ...
  $('tr').each(function sasa(i, item) {
    ...
    const [statements, branches, functions, lines] = $('.pct', item).text().split('%');
    ...
    .reduce((acc, file) => {
      ...
      if (res.length > 1) {
        value = res.reduce(
          (accumulator, item) => ({
            ...accumulator,
            statements: accumulator.statements + item.statements / res.length,
            branches: accumulator.branches + item.branches / res.length,
            functions: accumulator.functions + item.functions / res.length,
            lines: accumulator.lines + item.lines / res.length,
          }),
          { statements: 0, branches: 0, functions: 0, lines: 0 },
        );
      }

      const cov = Object.keys(value).reduce((accum, typeName) => accum + value[typeName], 0);
  ...
  return {
    ...
    pkg: {
      ...
      cov: Object.keys(jestCov).reduce((acc, k) => {
        /* istanbul ignore next */
        const newKey = k.endsWith('src') ? k.split('/').slice(0, -1).join('/') : k;
...
function tsconfigResolver(rootDir) {
  ...
  return isMono
    ? [project].concat(workspaces.reduce((acc, ws) => acc.concat(path.join(cwd, ws, '*', 'tsconfig.json')), []))
    : project;
}
